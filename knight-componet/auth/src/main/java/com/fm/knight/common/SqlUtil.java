...
public class SqlUtil {
    ...
    public static String formatSql(String sql) throws Exception {
        ...
        if ("select from".equalsIgnoreCase(sql) || sql.toLowerCase().endsWith(" distinct from"))
            sql = sql.replace(" from", "  from");
        if ("select".equalsIgnoreCase(sql) || "select distinct".equalsIgnoreCase(sql) || sql.toLowerCase().endsWith(" from"))
            sql += " ";
        return sql;
    }
    ...
    public static String oracle2Sqlserver(String sql) throws Exception {
        String result = sql.replace("nvl(", "isnull(").replace("||", "+").replace("chr(", "char(").replace("ceil(", "ceiling(").replace("length(", "len(").replace("substr(", "substring(");
        //TODO 转换sysdate + n
        //...
    ...
    public static String concat(Object... params) throws Exception {
        ...
        for (int i = 0; i < params.length; i++) {
            ...
            if (params[i] instanceof String[]) {
                ...
                if (("," + strs[0] + ",").indexOf("," + sqlType + ",") != -1) des = to_char(strs[1], strs[2]);
                else des = "" + strs[1];
            }
            des = convertSql(des);
            if (SQLSERVER.equals(sqlType) || MYSQL.equals(sqlType)) {
